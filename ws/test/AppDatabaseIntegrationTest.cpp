...
const std::filesystem::path kConfigPath =
    std::filesystem::temp_directory_path() / "app_database_integration_test_config.json";
...
std::filesystem::path find_repo_root() {
  ...
  while (!current.empty()) {
    if (std::filesystem::exists(current / "sql" / "tables.sql") &&
        std::filesystem::exists(current / "sql" / "indexes.sql")) {
      return current;
    }
      ...
      {"database",
       {{"app",
         {{"enabled", true},
          ...
          {"schema",
           {{"tables_path", (repo_root / "sql" / "tables.sql").string()},
            {"indexes_path", (repo_root / "sql" / "indexes.sql").string()}}}}}}},
      {"server",
       {{"host", "127.0.0.1"},
        {"port", 228},
...
std::string make_auth_header() {
  ...
  return "Bearer " +
         infrastructure::auth::create_jwt(user, cfg, "app-database-integration-test");
}
...
std::optional<std::string> catalog_parameter_value(std::string_view device_id,
...
void wait_for_persisted_parameter(std::string_view device_id,
                                  std::string_view name,
...
void test_app_seeds_catalog_and_reloads_persisted_parameters() {
...

  {
    ...
    expect(devices_it != list_response.payload.end() && devices_it->is_array(),
           "device.list should return devices array");
    expect(devices_it->size() == 3,
           "db-backed app should seed three emulated devices on first startup");

    client.write_message(make_request(
  ...
  expect(catalog_device_count() == 3,
         "first startup should persist the default emulated fleet");
  const auto persisted_gain = catalog_parameter_value(kDeviceId, "gain");
  expect(persisted_gain.has_value() && *persisted_gain == "42",
         "parameter update should be stored in the catalog");

  {
    ...
    const auto devices_it = list_response.payload.find("devices");
    expect(devices_it != list_response.payload.end() && devices_it->is_array(),
           "device.list should return devices array after reload");
    expect(devices_it->size() == 3,
           "reloading from the catalog should not duplicate seeded devices");
    ...
    const auto parameter_snapshot_event =
        client.await_event(kParameterTopic, kDeviceId);
    expect(parameter_snapshot_event.payload["snapshot"] == true,
           "reloaded device should emit a snapshot event for persisted parameter");
    expect(parameter_snapshot_event.payload["value"] == "42",
           "reloaded device should restore persisted parameter values");
  ...
  expect(catalog_device_count() == 3,
         "restarting with persisted devices should keep the catalog cardinality stable");
  reset_catalog();
}
