...
public:
  ...
  template <typename U>
  PutResultErrorCode put_wait(U&& task) {
    ...
    while (Ops::size(m_buffer) + 1 > m_maxQueueSize && m_maxQueueSize != 0) {
      ...
      m_notFull.wait(lock, [this]() { return m_state.load() == ContainerState::closing || (Ops::size(m_buffer) < m_maxQueueSize); });
      if (m_state.load() == ContainerState::closing) {
        return PutResultErrorCode::closed;
      }
    }
  ...
  template <typename U>
  PutResultErrorCode put_wait_for(U&& task, size_t ms) {
    std::unique_lock<std::mutex> lock(m_mutex);
    if (m_state.load() != ContainerState::open) {
      return PutResultErrorCode::closed;
    }
    ...
    while (Ops::size(m_buffer) + 1 > m_maxQueueSize && m_maxQueueSize != 0) {
      ...
      bool wait_result = m_notFull.wait_until(lock, deadline, [this]() { return m_state.load() == ContainerState::closing || (Ops::size(m_buffer) < m_maxQueueSize); });
      if (!wait_result) {
  ...
  TakeResult<T> tryTake(size_t time_out_ms = 100) {
    ...
    while (Ops::empty(m_buffer)) {
      if (!m_notEmptyQueue.wait_for(lock, duration, [this] { return m_state.load() == ContainerState::closing || !Ops::empty(m_buffer); })) { // wakes when stop requested or not empty
        // this is timeout path
        return TakeResult<T>{.erc = TakeResultErrorCode::timeout}; // timed out // timed out
                                                                   // and still empty
      }
      // no-timeout path
